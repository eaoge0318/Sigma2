...
async function runFullSimulation() {
    ...
    // 清空前端顯示狀態
    lastLogTimestamp = 0;
    document.getElementById('reasoning-logs').innerHTML = '<div id="log-empty-msg" style="color: #94a3b8; text-align: center; padding: 30px;">目前沒有數據，請先啟動系統以收集數據。</div>';
...
function switchTrainingMainTab(tab) {
    const buildContent = document.getElementById('training-main-build-content');
    const registryContent = document.getElementById('training-main-registry-content');
    const btnBuild = document.getElementById('tab-train-main-build');
    const btnRegistry = document.getElementById('tab-train-main-registry');

    if (tab === 'build') {
        if (buildContent) buildContent.style.display = 'block';
        if (registryContent) registryContent.style.display = 'none';
        setActiveTabStyle(btnBuild, [btnRegistry]);
        // 預設切換到子分頁的「配置」
        switchTrainingSubTab('config');
        clearAllTrainingTimers();
    } else {
        if (buildContent) buildContent.style.display = 'none';
        if (registryContent) registryContent.style.display = 'block';
        setActiveTabStyle(btnRegistry, [btnBuild]);
        loadModelRegistry();
...
function switchTrainingSubTab(tab) {
    const configContent = document.getElementById('training-sub-config-content');
    const reportContent = document.getElementById('training-sub-report-content');
    const btnConfig = document.getElementById('tab-train-sub-config');
    const btnReport = document.getElementById('tab-train-sub-report');
    const viewTitle = document.getElementById('training-view-title');

    if (tab === 'config') {
        if (configContent) configContent.style.display = 'block';
        if (reportContent) reportContent.style.display = 'none';
        if (viewTitle) viewTitle.innerText = '配置訓練參數';
        setActiveTabStyle(btnConfig, [btnReport]);
        ...
        setActiveTabStyle(btnReport, [btnConfig]);
    }
}
...
async function loadModelRegistry() {
    ...
    try {
        const response = await fetch('/api/analysis/list_models?session_id=' + SESSION_ID);
