...
function initEventListeners() {
    ...
    goalInputs.forEach(id => {
        DOM.on(id, 'input', () => {
            const target = parseFloat(DOM.val('goal-target') || 0);
            const usl = parseFloat(DOM.val('goal-usl') || 0);
            const lsl = parseFloat(DOM.val('goal-lsl') || 0);
            updateGoalChartLines(target, usl, lsl);
        });
    });
    initGoalChartDrag();
}
...
// (ESC Handler moved to initEventListeners)
